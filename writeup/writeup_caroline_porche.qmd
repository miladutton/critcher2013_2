...
processed_data <- combined_data %>%
  ...
  # Add 'condition' column based on stimulus content
  ...
  # Propagate 'condition' values downward to fill in any gaps
  fill(condition, .direction = "down") %>%
  
  # Organize data by 'subject_id'
  group_by(subject_id) %>%
  
  # Add attention check based on the last row's response
  mutate(attention_check = if_else(
    row_number() == n(),
    str_extract(response, '"Q0":\\d+') %>% str_extract("\\d+$"),
    NA_character_
  )) %>%
...
expanded_data <- processed_data %>%
  mutate(parsed_response = map(response, ~ fromJSON(.) %>% as.data.frame())) %>%
  unnest(cols = parsed_response) %>%
  
  # Add a 'target' column to label data for Justin and Nate
  mutate(target = case_when(
    row_number() %% 2 == 1 ~ "Justin",
    TRUE ~ "Nate"
  ))
...
# Visualize the results and Plot the Grouped Bar Chart
ggplot(filtered_long_data, aes(x = decision, y = moral_character_evaluation, fill = speed)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8), width = 0.7) +
  labs(
    title = "Moral Character Evaluation by Decision and Speed",
    x = "Decision",
    y = "Moral Character Evaluation"
  ) +
  scale_fill_manual(values = c("Quick" = "#8B008B", "Slow" = "#FF00FF")) + # Dark Purple and Magenta
  theme_minimal() +
  theme(
...
model <- aov(moral_character ~ decision_speed * decision, data = reshaped_data)
summary(model)
